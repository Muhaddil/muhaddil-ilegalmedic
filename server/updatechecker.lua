...
local resourceRepo = 'Muhaddil/muhaddil-ilegalmedic'
    ...
    PerformHttpRequest(githubApiUrl, function(statusCode, response, headers)
        if statusCode == 200 then
            ...
            if data and data.tag_name then
                ...
                printWithColor('[muhaddil-ilegalmedic] - Error: Invalid JSON structure.', '31') -- Red
            end
        else
            printWithColor('[muhaddil-ilegalmedic] - Failed to fetch version data. Status code: ' .. statusCode, '31') -- Red
        end
    end, 'GET')
end
...
function displayVersionData()
    ...
    if versionData.latestVersion then
        if versionData.latestVersion ~= currentVersion then
            print('╭────────────────────────────────────────────────────╮')
            printCentered('[muhaddil-ilegalmedic] - New Version Available', boxWidth, '34') -- Blue
            ...
            print('╭────────────────────────────────────────────────────╮')
            printWrapped('[muhaddil-ilegalmedic] - Up-to-date', boxWidth, '32')  -- Green
            printWrapped('Current version: ' .. currentVersion, boxWidth, '32') -- Green
            print('╰────────────────────────────────────────────────────╯')
        end
    else
        printWithColor('[muhaddil-ilegalmedic] - No version data available.', '31') -- Red
    end
end
